  ...
  return (
    <div className="space-y-16 pb-20 overflow-x-hidden">
      ...
      <section id="layanan" className="scroll-mt-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 contain-content-auto">
        ...
        {/* Category Filter Tabs: Semua Layanan + 11 kategori */}
        <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
          ...
          >
            <Layers className={`w-3.5 h-3.5 ${activeCategory === "all" ? "text-hk-champagne" : "text-hk-taupe"}`} />
            <span>Semua Layanan</span>
          </button>
          {VENDOR_CATEGORIES.map((cat) => {
            const CatIcon = CATEGORY_ICONS[cat.iconName] ?? Store;
            const isActive = activeCategory === cat.id;
            const count = vendorsByCategory[cat.id]?.length ?? 0;
            return (
              <button
                key={cat.id}
              ...
              >
                <CatIcon className={`w-3.5 h-3.5 ${isActive ? "text-hk-champagne" : "text-hk-taupe"}`} />
                <span>{cat.title}</span>
                {count > 0 && (
                  <span
