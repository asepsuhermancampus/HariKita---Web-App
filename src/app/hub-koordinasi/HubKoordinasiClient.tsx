...
export function HubKoordinasiClient({ dbOrders = [] }: { dbOrders?: OrderSummaryDTO[] }) {
  const searchParams = useSearchParams();
  const urlBookingId = searchParams.get("bookingId");
  const { orders: mockOrders } = useOrders();

  // Sumber acara: DB (bila ada) → mock fallback (bentuk disamakan).
  const orders =
    dbOrders.length > 0
      ? dbOrders.map((o) => ({
          bookingId: o.orderNumber,
          customerName: o.clientName,
          eventDate: o.eventDate.split("T")[0],
        }))
      : mockOrders.map((o) => ({
