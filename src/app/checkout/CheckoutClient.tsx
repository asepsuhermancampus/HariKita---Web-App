...
export function CheckoutClient({ settings }: { settings: CheckoutSettings }) {
  ...
  const {
    ...
    removeItem,
    clearCart,
  } = useCart({ dpPct: settings.dpPct, platformFeePct: settings.platformFeePct });
  ...
  const dpWithFee = Math.round((totalWithFee * settings.dpPct) / 100);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                >
                  <div className="space-y-1">
                    <span className="font-editorial text-lg font-bold text-hk-charcoal block">
                      DP {settings.dpPct}% Kunci Tanggal
                    </span>
                    <p className="text-[11px] font-manrope text-hk-charcoal/70">
                      Cukup bayar {settings.dpPct}% sekarang untuk mengunci jadwal vendor. Pelunasan{" "}
                      {100 - settings.dpPct}% di H-7 acara.
                    </p>
                  </div>
        ...
        <div className="lg:col-span-5 p-6 rounded-3xl bg-hk-ivory/60 border border-hk-champagne/60 shadow-sm space-y-4">
          ...
          <div className="pt-3 border-t border-hk-champagne/40 space-y-2 text-xs font-manrope">
            ...
            <div className="flex justify-between text-hk-charcoal/70">
              <span>Biaya Platform &amp; Server ({platformFeePct}%):</span>
              <span className="font-mono font-bold text-hk-charcoal">
                Rp {platformFeeAmount.toLocaleString("id-ID")}
              </span>
            </div>
            ...
            <div className="flex justify-between font-bold text-emerald-800 pt-1">
              <span>Tagihan DP {settings.dpPct}% Sekarang:</span>
              <span className="font-mono text-sm">
                Rp {dpWithFee.toLocaleString("id-ID")}
              </span>
            </div>
            <div className="flex justify-between text-hk-charcoal/60 text-[11px]">
              <span>Sisa Pelunasan {100 - settings.dpPct}% (H-7):</span>
              <span className="font-mono">
                Rp {(totalWithFee - dpWithFee).toLocaleString("id-ID")}
              </span>
            </div>
          </div>
          ...
          <div className="p-3 rounded-xl bg-emerald-50 border border-emerald-200 flex items-start gap-2 text-[11px] font-manrope text-emerald-900">
            ...
            <span>
              Dana DP aman di Rekening Bersama resmi HariKita. {settings.dpPct}% hak vendor dicairkan pada H-3 untuk persiapan bahan.
            </span>
          </div>
        </div>
      </div>
