...
interface PageProps {
  ...
  /** Data order dari database (bila tersedia) — prioritas di atas mock store. */
  initialOrder?: OrderViewModel | null;
}

export function InvoiceClient({ params, initialOrder = null }: PageProps) {
  ...
  useEffect(() => {
    // Prioritas: data database → mock store lokal.
    if (initialOrder) {
      setOrder({
        id: initialOrder.id,
        bookingId: initialOrder.bookingId,
        customerName: initialOrder.customerName,
        customerWhatsApp: initialOrder.customerWhatsApp,
        eventDate: initialOrder.eventDate,
        eventLocation: initialOrder.eventLocation,
        district: initialOrder.district,
        paymentType: initialOrder.paymentType,
        paymentStatus: initialOrder.paymentStatus,
        notes: initialOrder.notes,
        financials: {
          subtotal: initialOrder.financials.subtotal,
          platformFee: initialOrder.financials.platformFee,
          escrowFee: 0,
          totalAmount: initialOrder.financials.totalAmount,
          dpAmount: initialOrder.financials.dpAmount,
          pelunasanAmount: initialOrder.financials.pelunasanAmount,
        },
        ...
        rundown: [],
        escrowStatus: {
          dpReleased: initialOrder.escrowStatus.dpReleased,
          settlementReleased: initialOrder.escrowStatus.settlementReleased,
        },
    ...
    setOrder(found);
  }, [bookingId, getOrderById, initialOrder]);
