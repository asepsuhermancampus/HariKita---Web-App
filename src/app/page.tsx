...
export default function HomePage() {
  ...
  const filteredCategories = React.useMemo(() => {
    if (activePhase === "all") return VENDOR_CATEGORIES;
    return VENDOR_CATEGORIES.filter((cat) => cat.phase === activePhase);
  }, [activePhase]);
      ...
      <section id="layanan" className="scroll-mt-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 contain-content-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCategories.map((item) => {
            const Icon = CATEGORY_ICONS[item.iconName] ?? Store;
            const vendorCount = getVendorsByCategory(item.id).length;
            return (
              <div
                key={item.id}
              ...
              >
                ...
                <div className="pt-5 mt-4 border-t border-hk-champagne/40 flex items-center justify-end">
                  <Link
