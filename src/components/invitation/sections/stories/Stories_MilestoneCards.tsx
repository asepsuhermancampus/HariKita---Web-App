  ...
  return (
    <section id="story" className="py-24 px-4 sm:px-6 relative overflow-hidden">
      <div className="max-w-3xl mx-auto space-y-16">
        ...
        <div className="relative border-l-2 border-dashed border-amber-300/60 ml-3.5 sm:ml-8 pl-5 sm:pl-8 space-y-10">
          {stories.map((item, idx) => (
            <div key={idx} className="relative group">
              {/* Timeline Marker Ring */}
              <div
                className="absolute -left-[32px] sm:-left-[43px] top-2 w-5 sm:w-6 h-5 sm:h-6 rounded-full border-2 sm:border-4 border-white shadow-md flex items-center justify-center transition-transform group-hover:scale-125"
              ...
              <div className="bg-white/95 backdrop-blur-md p-5 sm:p-7 rounded-3xl border border-slate-200/80 shadow-xl space-y-3 hover:-translate-y-1 transition-transform">
                <span className="text-[11px] font-bold font-mono uppercase tracking-wider px-3 py-1 rounded-full bg-amber-100 text-amber-900 inline-block">
                  Tahun {item.year}
                </span>
                <h3 className="text-lg sm:text-xl font-serif font-bold text-slate-900 leading-snug">{item.title}</h3>
