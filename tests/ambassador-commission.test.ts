...
test("requestWithdrawal deducts wallet balance", async () => {
  ...
  await requestWithdrawal({ ambassadorId: ba.ambassadorId, amount: 40_000 }, prisma);
  const wallet = await prisma.brandAmbassador.findUnique({ where: { id: ba.ambassadorId } });
...
test("requestWithdrawal rejects amount above balance", async () => {
  ...
  await assert.rejects(() => requestWithdrawal({ ambassadorId: ba.ambassadorId, amount: 20_000 }, prisma));
});
...
test("resolveWithdrawal REJECTED restores balance", async () => {
  ...
  const { withdrawalId } = await requestWithdrawal({ ambassadorId: ba.ambassadorId, amount: 50_000 }, prisma);
